...
g_ESRI_variable_1 = u'extractFeatures_SHP'
...
class ReportGenerator():
    ...
    def run(self, inputJSON):
        try:
            ...
            if "CSV" == iReader.getReportFormat():
                outputURL = self._generateCSV(iReader)
            elif "SHP" == iReader.getReportFormat():
                outputURL = self._generateSHP(iReader)
            elif "KMZ" == iReader.getReportFormat():
                outputURL = self._generateKMZ(iReader)
            else:
                raise Exception('CSV, KMZ, and SHP are the only implemented report formats. Requested: ' + str(
                    iReader.getReportFormat()))
...
class FeatureReader():
    ...
    def getFeatures(self, url, objectIds, fields, outFeatureClass=''):
        try:
            if ('' == outFeatureClass): outFeatureClass = arcpy.CreateScratchName("Feat_", "", "FeatureClass",
            ...
            assert "esriGeometryPolygon" == esriJSON['geometryType'], str(
                "Only polygons are supported. Requested: " + str(esriJSON['geometryType']))

            tempFeatureClassName = arcpy.CreateScratchName("tmp_", "", "FeatureClass")
            print tempFeatureClassName

            FeatureClass = (
                arcpy.CreateFeatureclass_management(arcpy.env.scratchWorkspace, tempFeatureClassName, "POLYGON"))
    ...
    def parseJSON(self, response, objectIds="", fields=""):
        ...
        try:
            ...
            for jFeature in jsonDict["features"]:
                featureFields = {}
                fid = self.get_feature_id(jFeature)
                if fid is None:
                    continue
                featureID = str(fid).replace("u'", "'")
                for i, jfield in enumerate(fieldNames):
                    featureFields[str(jfield).replace("u'", "'")] = str(jFeature["attributes"][fieldNames[i]]).replace(
                        "u'", "'")
...
class ReportWriter():
    ...
    def _writeSHP(self, fObject, name, folder):
        try:
            ...
            Output_Layer = g_ESRI_variable_1
...
class Zipper():
    ...
    def _addShapeFiles(self, path, zip, keep, name=""):
        ...
        shpFormats = ['.shp', '.shx', '.dbf', '.sbn', '.sbx', '.fbn', '.ain', '.aih', '.ixs', '.mxs', '.atx', '.xml',
                      '.cpg', 'prj']
...
class FieldObject(FeatureObject):
    ...
    def get_feature_id(self, feature):
        ...
        if 'OBJECTID' in feature["attributes"]:
            key = 'OBJECTID'
        elif 'objectid' in feature["attributes"]:
            key = 'objectid'
        elif 'objectID' in feature["attributes"]:
            key = 'objectID'
        elif 'objectId' in feature["attributes"]:
    ...
    def _validateFields(self, fieldList):
        ...
        if ([] != self.orderedFields):
            assert 0 == len(set(self.orderedFields).difference(set(fieldList))), str(
                "Input fields don't match existing fields. Input: " + str(self.orderedFields) + ", Output: " + str(
                    fieldList))
        else:
